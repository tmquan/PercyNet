    ...
    def build_graph(self, I1, J2):
        ...
        with tf.variable_scope('gen'):
            with tf.variable_scope('encoder'):
                ...
                ZZf = tf.zeros_like(C1f)

                CZf = tf.concat([C1f, ZZf], axis=-1)
                ZSf = tf.concat([ZZf, S2f], axis=-1)
                CSf = tf.concat([C1f, S2f], axis=-1)
            ...
            with tf.variable_scope('decoder'):
                I1C = self.decoder(CZf)
                J2S = self.decoder(ZSf)
                C   = self.decoder(CSf)
            ...
            I1C  = tf_2imag(I1C, maxVal=255.0)
            J2S  = tf_2imag(J2S, maxVal=255.0)
            I1r  = tf_2imag(I1r, maxVal=255.0)
            J2r  = tf_2imag(J2r, maxVal=255.0)

            C  = tf.identity(C, name='C')
            I1C  = tf.identity(I1C, name='I1C')
            J2S  = tf.identity(J2S, name='J2S')
            I1r  = tf.identity(I1r, name='I1r')
            J2r  = tf.identity(J2r, name='J2r')

            with tf.variable_scope('encoder'):
                I1Cf = self.encoder(I1C)
                J2Sf = self.encoder(J2S)
        ...
        viz = tf.concat([
                        tf.concat([I1, J2, I1C, J2S], axis=2), 
        ...
        with tf.name_scope('losses'):
            ...
            with tf.name_scope('recon'):
                ...
                recon_loss_I1Cf = tf.reduce_mean(tf.abs(I1Cf - CZf), name='recon_loss_I1Cf')
                recon_loss_J2Sf = tf.reduce_mean(tf.abs(J2Sf - ZSf), name='recon_loss_J2Sf')
                g_losses.append(GAMMA*recon_loss_I1)
                g_losses.append(GAMMA*recon_loss_J2)
                g_losses.append(recon_loss_I1Cf)
                g_losses.append(recon_loss_J2Sf)
        ...
        add_moving_summary(recon_loss_I1,
                           recon_loss_J2,
                           recon_loss_I1Cf,
                           recon_loss_J2Sf,
                           # tv_loss,
